...
class cCapacitacionActAdm{
	...
	public function tablaCapacitacionDocenteActDis($idUsr,$tipoForm){
		$resp = "";
		$respBd;
		$mysqli = new mysqli("localhost","root","","curriculum");
		...
		if($mysqli){
			...
			if($respBd->num_rows > 0){//si hay una respuesta por parte de la bd
					   ...
					   while($row = $respBd->fetch_assoc()){
						   $resp = $resp."<tr>";
								...
								if($tipoForm == "Cap"){
									...
									if($tipoForm == "Cap"){
										$resp = $resp."<td>";
										$resp = $resp."<a href="."?a=".$this->codificaURL("1")."&t=".$this->codificaURL($row['tipoC'])."&i=".$this->codificaURL($row['institucion'])."&p=".$this->codificaURL($row['pais'])."&an=".$this->codificaURL($row['anio'])."&h=".$this->codificaURL($row['horas'])."&tl=0"."&idC=".$this->codificaURL($row['idCapacitacion']).">Actualizar</a>";
									$resp = $resp."</td>";
									}else{
										$resp = $resp."<td>";
									...
									if($tipoForm == "Cap"){
									$resp = $resp."<td>";
									$resp = $resp."<a onclick=\"return confirm('¿Está seguro de eliminar el registro?')\"".' href='."?tl=1&e=".$this->codificaURL('1')."&idC=".$this->codificaURL($row['idCapacitacion']).">Eliminar</a>";
									$resp = $resp."</td>";
									}else{
									$resp = $resp."<td>";
					   ...
					   $resp = $resp."</table></div>";
						$resp = $resp."<a href=?tl=0>Regresar</a>	";
